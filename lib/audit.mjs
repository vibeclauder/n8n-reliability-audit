...
// Severities ordered least → most serious. Used to decide the process exit
// code when the tool runs in a CI gate: a `--fail-on <severity>` threshold
// fails the build when any finding at that severity or higher is present.
// `info` is deliberately excluded as a gate threshold — informational
// findings (e.g. a disabled node) are not actionable failures, so gating on
// them would be permanently red and meaningless.
const GATE_SEVERITIES = ['low', 'medium', 'high', 'critical'];
...
// AI/LLM node type fragments. These are treated separately from generic
// integrations because LLM providers fail transiently far more often
// (rate limits, model overload, long tail latency) and because agent nodes
// can loop. Detection is by type-string fragment so it survives the
// `@n8n/n8n-nodes-langchain.*` namespace and vendor-specific community nodes.
const AI_TYPES = [
  'langchain',
  'openai',
  'anthropic',
...
function looksLikeAiNode(node) {
  const type = String(node.type || '').toLowerCase();
  return AI_TYPES.some(token => type.includes(token));
}

function looksLikeAiAgent(node) {
  const type = String(node.type || '').toLowerCase();
...
function auditWorkflow(workflow, index) {
  ...
  const pinnedNodes = isObject(workflow.pinData) ? Object.keys(workflow.pinData) : [];
  if (pinnedNodes.length > 0) {
    findings.push(finding(
      ...
      name,
      null,
      `${pinnedNodes.length} node(s) carry pinned data that overrides live execution.`,
      'Remove pinned test data before deploying. A pinned node returns fixed captured output instead of running, so the workflow can silently serve stale data in production.'
    ));
  }
  ...
  for (const node of nodes) {
    ...
    if (looksLikeAiNode(node) && node.disabled !== true && node.retryOnFail !== true) {
      findings.push(finding(
        'high',
        'AI_NODE_NO_RETRY',
        name,
        node.name,
        'AI/LLM node has no retry policy for transient provider failures.',
        'Enable bounded retry with backoff. LLM providers return rate-limit (429) and overload (503/529) errors routinely; without retry these transient failures surface as full workflow failures.'
      ));
    }

    if (looksLikeAiAgent(node) && node.disabled !== true) {
      const params = isObject(node.parameters) ? node.parameters : {};
      const options = isObject(params.options) ? params.options : {};
      const iterationCap = options.maxIterations ?? params.maxIterations;
      if (iterationCap === undefined || iterationCap === null) {
        findings.push(finding(
          'medium',
          'AI_AGENT_NO_ITERATION_CAP',
          name,
          node.name,
          'AI agent node has no explicit maximum-iteration cap.',
          'Set an explicit max-iterations limit so a tool-using agent cannot loop indefinitely, run up token cost, or hang the execution.'
        ));
      }
    }

    if (String(node.type || '').toLowerCase().includes('httprequest') && node.disabled !== true) {
      ...
      if (timeout === undefined || timeout === null) {
        findings.push(finding(
          ...
          name,
          node.name,
          'HTTP request has no explicit timeout.',
          'Set an explicit request timeout so a slow or hung upstream cannot stall the execution and back up the queue.'
        ));
      }
    }
...
export const GATE_THRESHOLDS = GATE_SEVERITIES;

// Decide the CI gate result for a report. Returns { failed, threshold,
// gatedCount, gatedSeverities } so the CLI can both set an exit code and
// print a precise reason. With no `failOn` the default threshold is
// `critical`, preserving the original "exit 1 only on a critical finding"
// behavior. A `failOn` of e.g. `high` fails when any high OR critical finding
// exists. An unknown threshold throws so a typo in CI config surfaces loudly
// rather than silently passing.
export function evaluateGate(report, { failOn } = {}) {
  const threshold = failOn ?? 'critical';
  const index = GATE_SEVERITIES.indexOf(threshold);
  if (index === -1) {
    throw new Error(
      `Unknown --fail-on severity: ${threshold}. Use one of: ${GATE_SEVERITIES.join(', ')}.`
    );
  }
  const gatedSeverities = GATE_SEVERITIES.slice(index);
  const gatedCount = gatedSeverities.reduce(
    (sum, severity) => sum + (report.summary[severity] || 0),
    0
  );
  return { failed: gatedCount > 0, threshold, gatedCount, gatedSeverities };
}

// Convenience wrapper for callers that only want the numeric exit code.
export function gateExitCode(report, options = {}) {
  return evaluateGate(report, options).failed ? 1 : 0;
}
